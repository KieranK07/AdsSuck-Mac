...
ADSSUCK-MAC_VERSION="1.2.3.1115-1"
...
echo
echo "************************"
echo "AdsSuck-Mac by @KieranK07"
echo "************************"
echo

# Report versions
echo -e "Spotify version: ${CLIENT_VERSION}"
echo -e "AdsSuck-Mac version: ${ADSSUCK-MAC_VERSION}\n"
...
if [[ ! -f "${XPUI_SPA}" ]]; then
  ...
  if [[ "${FORCE_FLAG}" == "false" ]]; then
    if [[ -f "${XPUI_BAK}" ]]; then
      echo "AdsSuck-Mac backup found, AdsSuck-Mac has already been used on this install."
      echo -e "Re-run AdsSuck-Mac using the '-f' flag to force xpui patching.\n"
      echo "Skipping xpui patches and continuing AdsSuck-Mac..."
      XPUI_SKIP="true"
    else
...
if [[ "${XPUI_SKIP}" == "false" ]]; then
  ...
  if grep -Fq "AdsSuck-Mac" "${XPUI_JS}"; then
    echo -e "\nWarning: Detected AdsSuck-Mac patches but no backup file!"
...
echo "Applying AdsSuck-Mac patches..."
...
if [[ "${XPUI_SKIP}" == "false" ]]; then
  ...
  echo -e "\n//# AdsSuck-Mac was here" >> "${XPUI_JS}"; fi
...
echo -e "AdsSuck-Mac finished patching!\n"
